...
export default function SupervisorMyProjects() {
  ...
  const avgProgress =
    projects.length === 0
      ? 0
      : Math.round(
          projects.reduce((a, p) => a + projectProgress(p._id), 0) / projects.length
        );
      ...
      <main className="smp-main">
        ...
        <div className="ap-stats">
          <div className="ap-stat-card" style={{ borderLeftColor: "#14b8a6" }}>
            <div className="ap-stat-num" style={{ color: "#14b8a6" }}>{projects.length}</div>
          ...
          <div className="ap-stat-card" style={{ borderLeftColor: "#2563eb" }}>
            ...
            <div className="ap-stat-label">Ongoing</div>
          </div>
        ...
        {filtered.length === 0 ? (
          ...
          <div className="smp-grid">
            {filtered.map((project) => {
              const pri = PRIORITY_COLORS[project.priority] || PRIORITY_COLORS.Medium;
              const sta = STATUS_COLORS[project.status] || STATUS_COLORS.Pending;
              const progress = projectProgress(project._id);
              const pTasks = projectTasks(project._id);
              const progressColor =
                progress === 100
                  ? "#22c55e"
                  : progress >= 60
              ...
              return (
                <div
                  className="smp-card"
                  key={project._id}
                  onClick={() => navigate(`/supervisor/dashboard/${project._id}`)}
                >
                  ...
                  <h3 className="smp-card-name">{project.projectName}</h3>
                  ...
                  <div className="smp-card-footer">
                    <span className="smp-chip">{pTasks.length} tasks</span>
                    {project.endDate && (
                      <span className="smp-chip">
